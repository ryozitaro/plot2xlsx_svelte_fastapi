...
import { unitPrefixes, unitPrefixesKeys } from './unitPrefixes';
...
export const addPrefix = (bigString: string) => {
	...
	if (realNum === 0) {
		...
		return `${Big(`${realNum}e${newExp}`).toFixed()} ${expPrefix.prefix}`;
	}
};

export const displayNums = (str: string, custom?: (str: string) => string) =>
	str ? (custom ? custom(str) : str) : '---';
